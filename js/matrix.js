...
function matrix2table(arr, header=''){
    let rs = '', startNum = 0;
    if(typeof arr != 'undefined' && typeof arr[0] != 'undefined'){//데이터가 있어야 함.
...
//선택한 열 삭제
function deleteMatrixCol(arr, col){//type = num, name
    if(arr.constructor == Array && typeof arr[0] != 'undefined' && typeof arr[0][0] != 'undefined') {
        let cols= matrixColNames2indexs(arr, col)
        cols = cols.sort().reverse();
        // console.log(cols)
        for (let j = 0; j < cols.length; j++) {
            if(cols[j] > -1){
                arr.forEach(function(e, i){
                    e.splice(parseInt(cols[j]), 1)
                })
            }
        }
        return true;
    ...
    // function isArray(){ return (arr.constructor == Array) ? true : false; }
    // function isObject(){ return (arr.constructor == Object) ? true : false; }

    //col name to col index
    function matrixColNames2indexs(arr, colNames){
        if(typeof colNames == 'undefined') return ;
        // console.log(colNames, typeof colNames)
        if(colNames.constructor == Array) colNames = colNames.join();
        else colNames += '';
        colNames = colNames.replace(/ /g,'');
        let findStr = colNames.match(/[^\d,]+/g);
        let type = (findStr != null && findStr.length) > 0 ? 'name' : 'num'; //전체수자 또는 영어 한번만 들어가면 name으로 보기
        let cols = colNames.split(',')
        if(typeof colNames == 'string' && colNames.indexOf(',') > -1){//여러개
            for (let j = 0; j < cols.length; j++) {
                if(type !== 'num') cols[j] = parseInt(arr[0].indexOf(cols[j]))
                // console.log(cols[j])
            }
            // cols = cols.sort();
        }else{//한개
            if(type !== 'num') cols = [ parseInt(arr[0].indexOf(colNames)) ]
            else cols = [ parseInt(colNames) ];
        }
        // console.log(cols)
        return cols;
    }
...
//선택한 열 삽입
function insertMatrixCol(arr, col, val='', key=''){
    if(arr.constructor == Array && typeof arr[0] != 'undefined' && typeof arr[0][0] != 'undefined') {
        let cols= matrixColNames2indexs(arr, col)
        cols = cols.sort().reverse()
        // console.log(cols)
        for (let j = 0; j < cols.length; j++) {
            if(cols[j] > -1){
                arr.forEach(function(e, i){
                    e.splice(parseInt(cols[j]), 0, val)
                })
                if(key !== '') arr[0][cols[j]] = key;
            }
        }
    }
}
//insertMatrixCol(mainData, '2, 5','--key--','-----')
...
function matrixColum(arr, cols, isUnique=false){
    ...
    if(typeof cols != 'undefined' ){
        cols = matrixColNames2indexs(arr, cols)
        // console.log(cols)
        arr.forEach(function(row,i){
            let newRow = [];
            cols.forEach(function(useCol){
                if(useCol > -1) newRow.push(row[useCol])
            });
            if(newRow.length > 0) rs.push(newRow);
        });
...
//몇쨰 열 데이터 교체
function matrixColFill(arr, colIndex, val='', startRow=0, endRow=''){
    if(endRow == '') endRow = arr.length-1;
    if(colIndex > -1){
...
//범위만큼 수열을 만듬. (번호범위만큼)
function makeRangeNumArray(start, stop, step) {
    var a = [start], b = start;
    while (b < stop) {
        a.push(b += step || 1);
    }
...
// console.log(makeRangeNumArray(5, 30));// 5~30인 수 즉 [5,6,7,8,...29,30]

//개수만큼 수열 만듬. (같은 내용으로)
function makeSameValArray(total=0, val=''){
    if(total === 0) return [];
    let rs =[];
...
// -----------------------
    ...
    //##개수만큼 수자 채우기.
    function str_poundKeyNum(text, num=1){
        if(text.indexOf('#') > -1){
            let padNum = (text.match(/#/g) || [] ).length;
            // console.log(padNum,11111)
            text = text.replace(('').padStart(padNum, '#') , (''+num).padStart(padNum, '0') )
        }
        return text;
    }
    // str_poundKeyNum('#####',1); 
